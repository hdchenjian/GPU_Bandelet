...
__global__ void haar_2d_rows(float *image_dev, int width, int step){
	...
	if(threadIdx.x < width)
		cache[threadIdx.x] = image_dev[blockIdx.x * step + threadIdx.x];
	...
	if(threadIdx.x < size){
		...
		image_dev[blockIdx.x * step + threadIdx.x + size] = detail_coeff;
	}
}
...
__global__ void haar_2d_cols(float *image_dev, int height, int step){
	...
	if(threadIdx.x < height)
		cache[threadIdx.x] = image_dev[blockIdx.x + threadIdx.x * step];
	__syncthreads();		/*synchronize the threads, to get correct data*/
	...
	if(threadIdx.x < size){
		...
		image_dev[blockIdx.x + threadIdx.x * step + size * step] = detail_coeff;
	}
}
